RSpec.describe Peatio::stemycoin::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Peatio::stemycoin::Blockchain.new
      expect(blockchain1.features).to eq Peatio::stemycoin::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Peatio::stemycoin::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it 'custom feautures' do
      blockchain3 = Peatio::stemycoin::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::stemycoin::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::stemycoin::Blockchain.new }
  ...
  context :latest_block_number do
    ...
    let(:blockchain) do
      Peatio::Stemycoin::Blockchain.new.tap {|b| b.configure(server: server)}
    end
  ...
  context :build_transaction do
    ...
    context 'three vout tx' do
      ...
      let(:blockchain) do
        Peatio::Stemycoin::Blockchain.new.tap { |b| b.configure(currencies: [currency]) }
      end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Peatio::Stemycoin::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:server_without_authority) { 'http://127.0.0.1:19332' }
    let(:blockchain) do
      Peatio::Stemycoin::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
